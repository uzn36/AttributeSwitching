...
class GuassianDiffusion:
    ...
    def sample_from_reverse_process(
        self, model, xT, timesteps=None, model_kwargs={}, ddim=False, time = None
    ):
    ...
    def sample_remain_from_reverse_process(
        ...
        scalars = self.get_all_scalars(
            self.alpha_bar_scheduler, time_prev or timesteps, self.device, new_betas
        )#########

        time = int(timesteps / self.timesteps * time) #750 -> 375
        if time_prev:
            first = True
        ...
        for i, t in zip(np.arange(timesteps)[time::-1], new_timesteps[time::-1]): 
            ...
            with torch.no_grad():
                ...
                if i == 0:
                    ...
                    if ddim:
                        final = (
                            unsqueeze3x(scalars["alpha_bar"][current_sub_t_sub_1]).sqrt()
                            * pred_x0
                            + (
                                1 - unsqueeze3x(scalars["alpha_bar"][current_sub_t_sub_1])
                            ).sqrt()
                            * pred_epsilon
                        )
                    else:
                        final = pred_mean + unsqueeze3x(
                            scalars.beta_tilde[current_sub_t].sqrt()#current_sub_t_sub_1 + 1
                        ) * torch.randn_like(final)
                final = final.detach()
        return final
